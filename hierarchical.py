    ...
    def dist_matrix(self, another_cluster):
        ...
        for i in range(self.size):
            for j in range(another_cluster.size):
                dist[i][j] = cm.Building.Euclidean_dist(self.points[i], another_cluster.points[j])
        return dist
...
class hierarchical_cluster:
    ...
    def initialize(self, building_lst):
        ...
        for i in range(self.q):
            for j in range(i + 1, self.q):
                self.min_dist_matrix[i][j] = self.clusters_lst[i].min_dist(self.clusters_lst[j])
    ...
    def Adding(self, cluster1):
        ...
        self.min_dist_matrix = np.insert(self.min_dist_matrix, self.q - 1, vector1, axis=0)
        self.min_dist_matrix = np.insert(self.min_dist_matrix, self.q - 1, vector2, axis=1)
        self.q = len(self.clusters_lst)
...
def clustering(read_path, map, k=CENTERS_NUM, iteration=MAX_ITERATION):
    ...
    clustering_hier = hierarchical_cluster(clusters_lst=[], k=k)
    clustering_hier.initialize(building_lst)
    while clustering_hier.q > k:
        print("current num of clusters is:", clustering_hier.q)
        clustering_hier.clustering()
    return clustering_hier
