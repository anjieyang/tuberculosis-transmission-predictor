...
class SIRController:
    ...
    def simulate(self):
        for t in tqdm(range(1, self.model.time_steps), total=self.model.time_steps, desc='Simulating 1'):
    ...
    def simulate2(self):
        for t in tqdm(range(1, self.model.time_steps), total=self.model.time_steps, desc='Simulating 2'):
            # traverse the current group
            for current in range(len(self.model.sir_groups)):
                dS_current = 0
                current_group = self.model.sir_groups[current]
                ...
                # calculate the dS of the stay people
                dS_stay_current = b * stay_I_current * stay_S_current / stay_N_current

                # the dS_stay should never exceed stay S
                dS_stay_current = dS_stay_current if dS_stay_current <= stay_S_current else stay_S_current

                stay_S_current -= dS_stay_current
                dS_current += dS_stay_current
                ...
                for contact in range(len(self.model.sir_groups)):
                    ...
                    # assign the value
                    dS_current += dS_stay_current_by_out_contact + dS_out_current_by_stay_contact
        ...
        writer = pd.ExcelWriter(PATH_SIR + 'data2.xlsx', engine='xlsxwriter')
